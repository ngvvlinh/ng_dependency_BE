...
func (c *miscServiceProtobufClient) VersionInfo(ctx context.Context, in *cm.Empty) (*cm.VersionInfoResponse, error) {
	ctx = ctxsetters.WithPackageName(ctx, "handler")
...
func (c *webhookServiceProtobufClient) ResetState(ctx context.Context, in *ResetStateRequest) (*cm.Empty, error) {
	ctx = ctxsetters.WithPackageName(ctx, "handler")
	ctx = ctxsetters.WithServiceName(ctx, "Webhook")
	ctx = ctxsetters.WithMethodName(ctx, "ResetState")
	out := new(cm.Empty)
	err := doProtobufRequest(ctx, c.client, c.urls[0], in, out)
	if err != nil {
		return nil, err
...
func (c *webhookServiceJSONClient) ResetState(ctx context.Context, in *ResetStateRequest) (*cm.Empty, error) {
	ctx = ctxsetters.WithPackageName(ctx, "handler")
	ctx = ctxsetters.WithServiceName(ctx, "Webhook")
	ctx = ctxsetters.WithMethodName(ctx, "ResetState")
	out := new(cm.Empty)
...
func (s *webhookServiceServer) ServeHTTP(resp http.ResponseWriter, req *http.Request) {
	...
	case "/api/handler.Webhook/ResetState":
		s.serveResetState(ctx, resp, req)
		return
	default:
		msg := fmt.Sprintf("no handler for path %q", req.URL.Path)
		err = badRouteError(msg, req.Method, req.URL.Path)
...
func (s *webhookServiceServer) serveResetState(ctx context.Context, resp http.ResponseWriter, req *http.Request) {
	header := req.Header.Get("Content-Type")
	i := strings.Index(header, ";")
	if i == -1 {
		i = len(header)
	...
	switch strings.TrimSpace(strings.ToLower(header[:i])) {
	case "application/json":
		s.serveResetStateJSON(ctx, resp, req)
	case "application/protobuf":
		s.serveResetStateProtobuf(ctx, resp, req)
	default:
		msg := fmt.Sprintf("unexpected Content-Type: %q", req.Header.Get("Content-Type"))
		twerr := badRouteError(msg, req.Method, req.URL.Path)
		s.writeError(ctx, resp, twerr)
...
func (s *webhookServiceServer) serveResetStateJSON(ctx context.Context, resp http.ResponseWriter, req *http.Request) {
	var err error
	ctx = ctxsetters.WithMethodName(ctx, "ResetState")
	ctx, err = callRequestRouted(ctx, s.hooks)
	if err != nil {
	...
	reqContent := new(ResetStateRequest)
	unmarshaler := jsonpb.Unmarshaler{AllowUnknownFields: true}
	if err = unmarshaler.Unmarshal(req.Body, reqContent); err != nil {
		s.writeError(ctx, resp, wrapInternal(err, "failed to parse request json"))
		return
	...
	func() {
		defer ensurePanicResponses(ctx, resp, s.hooks)
		respContent, err = s.WebhookService.ResetState(ctx, reqContent)
	}()
	...
	if respContent == nil {
		s.writeError(ctx, resp, twirp.InternalError("received a nil *cm.Empty and nil error while calling ResetState. nil responses are not supported"))
		return
	}
...
func (s *webhookServiceServer) serveResetStateProtobuf(ctx context.Context, resp http.ResponseWriter, req *http.Request) {
	var err error
	ctx = ctxsetters.WithMethodName(ctx, "ResetState")
	ctx, err = callRequestRouted(ctx, s.hooks)
	if err != nil {
