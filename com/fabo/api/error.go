...
func InitHandleError(_bot *telebot.Channel) {
	bot = _bot
}

func HandleErrorFacebookAPI(body []byte, currentURL string) (_err error) {
	defer func() {
		if _err != nil {
			bot.SendMessage(fmt.Sprintf("[Error][Fabo]: %s", _err.Error()))
		}
	}()
...
func handleErrorFacebookAPI(facebookError *FacebookError, currentURL string) error {
