...
func ETop(env cmenv.EnvType) *whitelabel.WL {
	return &whitelabel.WL{
		...
		Config: whitelabel.Config{
			...
			InviteUserByEmailURL: ternary(env == cmenv.EnvProd,
				"https://etop.vn/invitation",
				"https://shop."+baseHost(env)+"/invitation",
			),
			InviteUserByPhoneURL: ternary(env == cmenv.EnvProd,
				"https://pos.etop.vn/invitation",
				"https://pos."+baseHost(env)+"/invitation",
			),
			SiteName:        "eTop",
			CompanyName:     "eTop",
