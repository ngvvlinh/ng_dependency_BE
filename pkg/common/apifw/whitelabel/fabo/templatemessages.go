...
func InitTemplateMsg() {
	...
	templatemessages.PhoneInvitationTpl = template.Must(template.New("phone-verification").Parse(`Bạn được mời vào cửa hàng {{.ShopName}} trên Faboshop. Hãy bấm vào liên kết sau để xác nhận {{.URL}}`))

	templatemessages.PhoneInvitationTplRepeat = template.Must(template.New("phone-verification").Parse(`Bạn được mời vào cửa hàng {{.ShopName}} trên Faboshop. Hãy bấm vào liên kết sau để xác nhận {{.URL}} (gửi lần {{.SendTime}})`))
}
