...
// FindRows build query and return sql.Rows. The caller must close after use.
func (q *queryImpl) FindRows(objs core.IFind, preds ...interface{}) (core.Opts, *sql.Rows, error) {
	query, args, err := q.BuildFind(objs, preds...)
	if err != nil {
		return q.opts, nil, err
	}
	rows, err := q.db.QueryContext(q.ctx, query, args...)
	if err != nil {
		return q.opts, nil, err
	}
	return q.opts, rows, nil
}
