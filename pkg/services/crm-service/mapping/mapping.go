...
func mapEtopToVtiger(e interface{}, mappingFieldMap map[string]string) (map[string]string, error) {
	...
	for n, i := v.NumField(), 0; i < n; i++ {
		...
		case "*timestamp.Timestamp":
			...
			t := pbcm.PbTimeToModel(ts)
			if t.IsZero() {
				continue
			}
			mapVtiger[key] = t.Format(timeLayout)
...
func (f *Mapper) MapingContactEtop2Vtiger(contact *crmservice.Contact) (map[string]string, error) {
	var mappingFieldMap = f.FieldMap["Contacts"]
	mapReturn, err := mapEtopToVtiger(contact, mappingFieldMap)
	return mapReturn, err
}

func (f *Mapper) MapingContactVtiger2Etop(v map[string]string) (*crmservice.Contact, error) {
	var mappingFieldMap = f.FieldMap["Contacts"]
...
func (f *Mapper) MappingTicketEtop2Vtiger(ticket *crmservice.Ticket) (map[string]string, error) {
	var mappingFieldMap = f.FieldMap["HelpDesk"]
	mapReturn, err := mapEtopToVtiger(ticket, mappingFieldMap)
	return mapReturn, err
}

func (f *Mapper) MappingTicketVtiger2Etop(v map[string]string) (*crmservice.Ticket, error) {
	var mappingFieldMap = f.FieldMap["HelpDesk"]
	var ticket crmservice.Ticket
