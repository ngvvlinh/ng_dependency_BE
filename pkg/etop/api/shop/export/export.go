...
func (s *ExportService) RequestExport(ctx context.Context, r *api.RequestExportRequest) (resp *api.RequestExportResponse, _err error) {
	...
	case "shop/orders":
		...
		if claim.AuthPartnerID == 0 && !s.Auth.Check(roles, string(acl.ShopOrderExport), isTest) {
			return nil, cm.Error(cm.PermissionDenied, "", nil)
		}
	case "shop/fulfillments":
		if claim.AuthPartnerID == 0 && !s.Auth.Check(roles, string(acl.ShopFulfillmentExport), isTest) {
			return nil, cm.Error(cm.PermissionDenied, "", nil)
		}

		for _, filter := range r.Filters {
			if filter.Name == "money_transaction.id" {
				if claim.AuthPartnerID == 0 && !s.Auth.Check(roles, string(acl.ShopMoneyTransactionExport), isTest) {
					return nil, cm.Error(cm.PermissionDenied, "", nil)
				}
				break
			}
