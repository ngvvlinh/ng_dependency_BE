...
func (s *WebServerService) populateWsWebSiteWithProduct(ctx context.Context, args *api.WsWebsite) (*api.WsWebsite, error) {
	...
	if args.NewProduct != nil {
		for _, v := range args.NewProduct.ProductIDs {
			...
			args.NewProduct.Products = append(args.NewProduct.Products, product.PbShopProductWithVariants(mapProducts[v]))
		}
	}
	if args.OutstandingProduct != nil {
		for _, v := range args.OutstandingProduct.ProductIDs {
			...
			args.OutstandingProduct.Products = append(args.OutstandingProduct.Products, product.PbShopProductWithVariants(mapProducts[v]))
		}
	}
	return args, nil
}
	...
	for key, _ := range args {
		if args[key].NewProduct != nil {
			for _, v := range args[key].NewProduct.ProductIDs {
				...
				args[key].NewProduct.Products = append(args[key].NewProduct.Products, product.PbShopProductWithVariants(mapProducts[v]))
			}
		}
		if args[key].NewProduct != nil {
			for _, v := range args[key].OutstandingProduct.ProductIDs {
				...
				args[key].OutstandingProduct.Products = append(args[key].OutstandingProduct.Products, product.PbShopProductWithVariants(mapProducts[v]))
			}
		}
	}
	return args, nil
