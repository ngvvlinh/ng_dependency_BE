...
func PbOrderShipping(m *ordermodel.Order) *order.OrderShipping {
	...
	return &order.OrderShipping{
		...
		Weight:              cmapi.PbPtrInt32(m.TotalWeight),
		GrossWeight:         cmapi.PbPtrInt32(m.TotalWeight),
		Length:              cmapi.PbPtrInt32(item.Length),
		Width:               cmapi.PbPtrInt32(item.Width),
		Height:              cmapi.PbPtrInt32(item.Height),
...
func OrderShippingToModel(m *order.OrderShipping, mo *ordermodel.Order) error {
	...
	orderShipping := &ordermodel.OrderShipping{
		...
		Length:              cmapi.PatchInt32(0, m.Length),
		Width:               cmapi.PatchInt32(0, m.Width),
		Height:              cmapi.PatchInt32(0, m.Height),
	...
	mo.ShopCOD = cmapi.PatchInt32(mo.ShopCOD, m.CodAmount)
