...
func (s *WebhookService) CreateWebhook(ctx context.Context, r *CreateWebhookEndpoint) error {
	resp, err := s.WebhookInner.CreateWebhook(ctx, r.Context.Shop.ID, r.CreateWebhookRequest)
	r.Result = resp
	return err
}

func (s *WebhookService) DeleteWebhook(ctx context.Context, r *DeleteWebhookEndpoint) error {
	resp, err := s.WebhookInner.DeleteWebhook(ctx, r.Context.Shop.ID, r.DeleteWebhookRequest)
	r.Result = resp
	return err
}

func (s *WebhookService) GetWebhooks(ctx context.Context, r *GetWebhooksEndpoint) error {
	resp, err := s.WebhookInner.GetWebhooks(ctx, r.Context.Shop.ID)
	r.Result = resp
	return err
}
