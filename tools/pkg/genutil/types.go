...
func CheckType(typ types.Type, typs ...types.Type) (types.Type, error) {
	...
	if err != nil {
		return nil, generator.Errorf(err, "must be %v", err)
	}
	return result, nil
}
...
func checkType(typ types.Type, typs ...types.Type) (types.Type, error) {
	...
	case Named:
		named, ok := typ.(*types.Named)
		if !ok {
			return nil, generator.Errorf(nil, "named type")
		}
		result, err := checkType(named.Underlying(), typs[1:]...)
		if err != nil {
			return nil, generator.Errorf(err, "named %v", err)
		}
		return result, nil
	...
	case Pointer:
		...
		if !ok {
			return nil, generator.Errorf(nil, "pointer type")
		}
		result, err := checkType(ptr.Elem(), typs[1:]...)
		if err != nil {
			return nil, generator.Errorf(err, "pointer to %v", err)
		}
		return result, nil
	...
	case Slice:
		...
		if !ok {
			return nil, generator.Errorf(nil, "slice type")
		}
		result, err := checkType(sl.Elem(), typs[1:]...)
		if err != nil {
			return nil, generator.Errorf(err, "slice of %v", err)
		}
		return result, nil
	...
	case Map:
		mp, ok := typ.(*types.Map)
		if !ok {
			return nil, generator.Errorf(nil, "map type")
		}
		if len(typs) > 1 {
			if typs[1] != nil {
				_, err := checkType(mp.Key(), typs[1])
				if err != nil {
					return nil, generator.Errorf(err, "map[%v] type", err)
				}
			}
			typs = typs[1:]
		}
		...
		if err != nil {
			return nil, generator.Errorf(err, "map[%v] of %v", typs[0], err)
		}
		return result, nil
	...
	case Struct:
		...
		if !ok {
			return nil, generator.Errorf(nil, "struct type")
		}
		return st, nil
	}
	...
	if named, ok := typs[0].(*types.Named); ok {
		if len(typs) > 1 {
			return nil, generator.Errorf(nil, "unexpected type after named")
		}
		if typ != named {
			return nil, generator.Errorf(nil, "named type %v", named.String())
		}
