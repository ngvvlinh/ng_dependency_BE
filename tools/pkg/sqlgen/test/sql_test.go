...
import (
	...
	"etop.vn/backend/pkg/common/sql/sq/mock"
...
func InitSchema() {
	db.MustExec(`
		DROP TABLE IF EXISTS "user_info", "complex_info", "user" CASCADE;
        CREATE TABLE "user" (
            id TEXT PRIMARY KEY,
            name       TEXT,
...
func shouldQuery(db *sq.Database, sql string, args ...interface{}) []interface{} {
	...
	if err != nil {
		ll.Panic("Error", l.Error(err), l.String("sql", sql), l.Object("args", args))
	}
	return s
}
