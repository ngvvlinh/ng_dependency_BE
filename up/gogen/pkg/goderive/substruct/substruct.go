...
func (g *gen) genFuncFor(in, out types.Type) error {
	...
	p.P("")
	p.P("func %vFrom%v(ps []%v) []%v {", plural(capitalize(sOutStr)), plural(capitalize(sInStr)), inStr, outStr)
	p.In()
	...
	p.P("")
	p.P("func %vTo%v(ss []%v) []%v {", plural(capitalize(sOutStr)), plural(capitalize(sInStr)), outStr, inStr)
	p.In()
