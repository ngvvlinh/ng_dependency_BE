...
func (h AggregateHandler) HandleBuildUrlConnectPaymentGateway(ctx context.Context, msg *BuildUrlConnectPaymentGatewayCommand) error {
	result, err := h.inner.BuildUrlConnectPaymentGateway(msg.GetArgs(ctx))
	msg.Result = result
	return err
}

func (h AggregateHandler) HandleCancelTransaction(ctx context.Context, msg *CancelTransactionCommand) error {
	result, err := h.inner.CancelTransaction(msg.GetArgs(ctx))
	msg.Result = result
	return err
}

func (h AggregateHandler) HandleGetTransaction(ctx context.Context, msg *GetTransactionCommand) error {
	result, err := h.inner.GetTransaction(msg.GetArgs(ctx))
	msg.Result = result
	return err
}
