...
const Path_Etelecom_GetExtensionInfo = "/shop.Etelecom/GetExtensionInfo"
const Path_Etelecom_ListCallLogs = "/shop.Etelecom/ListCallLogs"
...
func (s *EtelecomServiceServer) parseRoute(path string, hooks httprpc.Hooks, info *httprpc.HookInfo) (reqMsg capi.Message, _ httprpc.ExecFunc, _ error) {
	switch path {
	case "/shop.Etelecom/GetExtensionInfo":
		msg := &externaltypes.GetExtensionInfoRequest{}
		fn := func(ctx context.Context) (newCtx context.Context, resp capi.Message, err error) {
			inner := s.builder()
			info.Request, info.Inner = msg, inner
			newCtx, err = hooks.RequestRouted(ctx, *info)
			...
			resp, err = inner.GetExtensionInfo(newCtx, msg)
			return
		}
		return msg, fn, nil
	case "/shop.Etelecom/ListCallLogs":
		msg := &externaltypes.ListCallLogsRequest{}
		fn := func(ctx context.Context) (newCtx context.Context, resp capi.Message, err error) {
			inner := s.builder()
			info.Request, info.Inner = msg, inner
			newCtx, err = hooks.RequestRouted(ctx, *info)
			...
			resp, err = inner.ListCallLogs(newCtx, msg)
			return
		}
		return msg, fn, nil
	default:
