...
func verifyArgsUsed(set *ProviderSet, used []*providerSetSrc) []error {
	...
	for _, imp := range set.Imports {
		...
		if !found {
			if imp.VarName == "" {
				...
				errs = append(errs, fmt.Errorf("unused provider set %q (%v)", imp.VarName, imp.PkgPath))
			}
		}
	}
	for _, p := range set.Providers {
		...
		if !found {
			errs = append(errs, fmt.Errorf("unused provider %q (%v)", p.Pkg.Name()+"."+p.Name, p.Pkg.Path()))
		}
	}
