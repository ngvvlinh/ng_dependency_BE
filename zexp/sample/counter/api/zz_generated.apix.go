...
func NewServer(builder interface{}, hooks ...httprpc.HooksBuilder) (httprpc.Server, bool) {
	...
	case func() CounterService:
		return NewCounterServiceServer(builder, hooks...), true
	default:
		return nil, false
	}
}
...
type CounterServiceServer struct {
	...
	builder func() CounterService
}

func NewCounterServiceServer(builder func() CounterService, hooks ...httprpc.HooksBuilder) httprpc.Server {
...
func (s *CounterServiceServer) WithHooks(hooks httprpc.HooksBuilder) httprpc.Server {
	result := *s
	result.hooks = httprpc.ChainHooks(s.hooks, hooks)
	return &result
}

func (s *CounterServiceServer) ServeHTTP(resp http.ResponseWriter, req *http.Request) {
...
func (s *CounterServiceServer) parseRoute(path string, hooks httprpc.Hooks, info *httprpc.HookInfo) (reqMsg capi.Message, _ httprpc.ExecFunc, _ error) {
	switch path {
	case "/counter.Counter/Counter":
		...
		fn := func(ctx context.Context) (newCtx context.Context, resp capi.Message, err error) {
			...
			resp, err = inner.Counter(newCtx, msg)
			return
		}
		return msg, fn, nil
	case "/counter.Counter/Get":
		msg := &GetRequest{}
		fn := func(ctx context.Context) (newCtx context.Context, resp capi.Message, err error) {
			inner := s.builder()
			info.Request, info.Inner = msg, inner
