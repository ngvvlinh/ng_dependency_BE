...
// +gen:apix
// +gen:swagger:doc-path=zext/sample/calc3

// +apix:path=/calc.Calc
type CalcService interface {
	Calc(context.Context, *CreateEquationRequest) (*Equation, error)
...
type CreateEquationRequest struct {
	// (+, -, * /)
...
func (m *CreateEquationRequest) String() string { return jsonx.MustMarshalToString(m) }
...
func (eq *Equation) ProcessCalc(num1 dot.NullString, num2 dot.NullString, op EquationOperator) {
	...
	fNum1, _ := strconv.ParseFloat(num1.Apply(""), 64)

	fNum2, _ := strconv.ParseFloat(num2.Apply(""), 64)
